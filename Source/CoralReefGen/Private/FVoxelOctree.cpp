...
#include "CoralGrowthManager.h" // Обязательно для доступа к FSpeciesParams и агрессии
...
void FVoxelOctree::InsertRecursive(FVoxelOctreeNode* Node, FIntVector NodePos, int32 Size, int32 Depth, FIntVector TargetPos, int32 SpeciesID, const TArray<FSpeciesParams>& SpeciesList, UInstancedStaticMeshComponent* MeshComp, int32 InInstanceIndex) {
    if (Depth == MaxDepth) {
        if (Node->bIsOccupied) {
            // ЛОГИКА ЗАХВАТА: Сравниваем силы
            ...
            if (NewAggro > CurrentAggro) {
                // Если новый вид сильнее, перехватываем владение
                Node->SpeciesID = SpeciesID;
                
                // Перекрашиваем уже существующий меш в этой точке
            ...
            Node->SpeciesID = SpeciesID;
            Node->InstanceIndex = InInstanceIndex; // Сохраняем индекс нового куба
        }
        return;
    }
...
void FVoxelOctree::ExpandTree(FIntVector TargetPos) {
    // Упрощенная логика расширения для примера
    ...
    // (Полная логика переноса старого корня в новый обычно требует итерации по детям)
    delete Root;
    Root = NewRoot;
